...
import { Search, MapPin, Loader2, ChevronLeft, ChevronRight, Filter, X } from 'lucide-react';
...
export const CandidateSearchView: React.FC = () => {
  ...
  const fetchJobs = async (
    ...
    try {
      ...
      setErrorMsg('No se pudieron cargar algunas ofertas laborales.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const ubicacionLabel = UBICACIONES.find((u) => u.value === ubicacion)?.label || ubicacion;
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 mb-8 shadow-xl">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-center gap-3 bg-white p-2 rounded-2xl shadow-lg">
          {/* Input Palabra Clave */}
          <div className="flex-1 relative flex items-center w-full">
            ...
            <input
              ...
              className="w-full h-12 pl-12 pr-4 bg-transparent focus:outline-none text-slate-900 placeholder:text-slate-400 text-sm font-medium"
            />
          </div>
          ...
          <Button
            ...
            className={`text-slate-600 hover:text-slate-900 h-12 px-4 rounded-xl ${showFilters ? 'bg-slate-100' : ''}`}
          >
          ...
          <Button
            type="submit"
            size="lg"
            className="w-full sm:w-auto rounded-xl px-8 shadow-md shadow-blue-600/20 bg-blue-600 hover:bg-blue-700 text-white font-semibold"
          >
            Buscar
          </Button>
        </form>
        ...
        {showFilters && (
          <div className="mt-4 p-5 bg-white rounded-2xl border border-slate-200 shadow-md text-left animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2 mt-2">
                ...
                <p className="text-xs text-slate-500 mt-1 pl-6">
                  Activa esto para ver ofertas internacionales de la red Remotive, independientemente de tu búsqueda de ciudad.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Resultados Info Bar (NO se muestra durante loading) */}
      {!isLoading && jobsData && jobsData.totalElements > 0 && (
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <h2 className="text-xl font-bold text-slate-800">
            {jobsData.totalElements} {jobsData.totalElements === 1 ? 'empleo encontrado' : 'empleos encontrados'}
          </h2>
      ...
      {isLoading ? (
        <div className="py-20 flex flex-col items-center justify-center">
          ...
          <p className="text-slate-600 font-medium text-lg">Buscando empleos...</p>
        </div>
      ) : !jobsData || jobsData.content.length === 0 ? (
        /* Estado Vacío */
        <div className="py-16 px-6 text-center bg-white rounded-2xl border border-slate-200 border-dashed max-w-2xl mx-auto shadow-sm">
          <div className="h-16 w-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <Search className="h-8 w-8 text-slate-400" />
          </div>
          <h3 className="text-lg font-bold text-slate-900 mb-2">
            No se encontraron ofertas en {ubicacionLabel}.
          </h3>
          <p className="text-slate-500 mb-6">
            Prueba otra palabra o busca en todo Perú.
          </p>
        ...
        /* Grid de Ofertas consistente con página pública */
        <>
